...
### Звонки (Call)
Используется Mango Office Api для работы со звонками. Каждый звонок сохраняется в таблицу `calls` – при завершении
звонка Mango отправляет нам данные через webhook. Как только сформировалась запись разговора, Mango также отдаёт
нам `recording` через webhook. Данные кладутся в таблицу в неизменном виде – так как отдает Mango API.
- `answered_at` – время ответа на звонок. Если не установлено – разговора не было
- `finished_at` - время завершения звонка. Длительность звонка в секундах - это разница между `finished_at` и `answered_at`
...
### Клиенты (Client)
В контексте нашей системы – это ученики
...
6. В PHP по возможности используй типизацию и избегай `mixed`
7. Избегай "шелухи" в PHP-коде:
   - не делай явного приведения типов `(string)`, `(int)` и т.д. "на всякий случай", если в этом нет реальной необходимости;
   - не делай `trim()` и другие лишние нормализации "на всякий случай";
   - учитывай, что в Laravel пустые строки обычно приводятся к `NULL`, поэтому предпочитай простые проверки вида `if (! $model->field)`.
8. Никогда не поддерживай legacy: не добавляй compatibility-ветки и сразу удаляй устаревшие обходные сценарии.
