...
@foreach ($report->clientLessons as $cl)
    {{ date('d.m.Y', strtotime($cl->lesson->date)) }} – @if ($cl->status === ClientLessonStatus::absent)
        <b>не был</b>
    @elseif ($cl->status === ClientLessonStatus::late)
        опоздал на {{ $cl->minutes_late }} мин.
    @else
        был
    @endif @if ($cl->lesson->topic)
        ({{ $cl->lesson->topic }})
    @endif
...
Вы можете ознакомиться с копией этого отчета в личном кабинете ученика https://lk.ege-centr.ru. Если у Вас возникли вопросы - пожалуйста, звоните +7 (495) 646-85-92. С уважением, учебная часть ЕГЭ-Центра.
