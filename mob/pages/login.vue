...
const step = {
  enterPhoneNumber: 0,
  rememberUser: 1,
  telegramBotInstructions: 2,
  verifyCode: 3,
}

const window = ref(rememberUser ? step.rememberUser : step.enterPhoneNumber)
...
async function onPhoneEnter() {
  ...
  nextTick(() => {
    window.value = (phone.telegram_id || phone.is_telegram_disabled)
      ? step.verifyCode
...
function otherUser() {
  window.value = step.enterPhoneNumber
...
async function onOtpFinish() {
  ...
  logInAndRemember(user, token, currentPhone.value!)
}
